...
const AssociationRequestEntryLembaga: React.FC<{
  data: PermintaanAsosiasi[];
}> = ({ data }) => {
  const router = useRouter();

  const handleButtonClick = () => {
    router.push('/lembaga/inbox/[slug]');
  };
  return (
    <div className="flex flex-col rounded-xl pl-1 pb-1 p-4 font-sans">
      ...
      {data.map((item: PermintaanAsosiasi, index: number) => (
        ...
        >
          ...
          <div className="flex justify-end pr-[28px] font-regular weight-400">
            <Button
...
export default AssociationRequestEntryLembaga;
