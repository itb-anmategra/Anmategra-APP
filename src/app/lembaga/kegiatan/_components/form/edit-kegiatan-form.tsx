...
const EventInputSchema = z.object({
  ...
  background_image: z
    .union([z.string().url('Harus berupa URL yang valid'), z.literal('')])
    .optional(),
  organogram_image: z
    .union([z.string().url('Harus berupa URL yang valid'), z.literal('')])
    .optional(),
...
const EditKegiatanForm = ({
  ...
  // ✅ useForm hook with default values from existing event
  const form = useForm<EventInputSchemaType>({
    ...
    defaultValues: {
      ...
      end_date:
        kegiatan.end_date && !isNaN(Date.parse(kegiatan.end_date))
          ? new Date(kegiatan.end_date).toISOString()
          : undefined,
      status: kegiatan.status,
      oprec_link: kegiatan.oprec_link ?? '',
      location: kegiatan.location ?? '',
      participant_limit: kegiatan.participant_limit ?? 1,
